  ...
  return (
    <div className="screen">
      <div className="screen-scroll">
        ...
        {/* Featured hero card */}
        <div className="hero-card" onClick={() => onOpen('material')}
             style={{ background: 'var(--grad-warm-metric)', aspectRatio: '16 / 10' }}>
          {/* Soft circles on right */}
          <svg style={{ position: 'absolute', right: -20, top: -10, width: '60%', height: '125%', pointerEvents: 'none' }}
               viewBox="0 0 200 220" preserveAspectRatio="xMidYMid slice">
            <circle cx="145" cy="78"  r="40" fill="#F4B8D3" opacity="0.92"/>
            <circle cx="175" cy="112" r="28" fill="#E09ACB" opacity="0.82"/>
            <circle cx="120" cy="118" r="22" fill="#D5B2EA" opacity="0.86"/>
            <circle cx="158" cy="148" r="20" fill="#C898E0" opacity="0.82"/>
            <circle cx="190" cy="64"  r="14" fill="#F9D0E3" opacity="0.92"/>
          </svg>

          <div style={{ position: 'relative', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', height: '100%' }}>
            <div className="tag" style={{ alignSelf: 'flex-start', background: 'rgba(255,255,255,0.28)', color: 'white', backdropFilter: 'blur(8px)' }}>
              Материал дня
            </div>
            <div>
              <div className="t-h3 c-inverse" style={{ maxWidth: '85%', fontWeight: 700 }}>
                Как питание влияет<br/>на организм и возраст
              </div>
              <div className="flex items-center justify-between" style={{ marginTop: 'var(--sp-3)' }}>
                <div className="t-body-sm c-inverse" style={{ opacity: 0.92 }}>6 минут</div>
                <button className="icon-btn" aria-label="Открыть"
        ...
        <div className="section-gap-sm">
          <div className="segmented">
            <button className={tab === 'practice' ? 'active' : ''} onClick={() => setTab('practice')}>Практики</button>
            <button className={tab === 'breath' ? 'active' : ''} onClick={() => setTab('breath')}>Дыхание</button>
          ...
          {items.map(it => (
            <div key={it.id} className="action-row" onClick={() => onOpen('article-' + it.id)}>
              ...
              <div className="row-body">
                ...
                <div className="t-label-md c-tertiary" style={{ marginTop: 2 }}>{it.dur}</div>
              </div>
    ...
    <svg viewBox="0 0 52 52" {...common}>
      <defs>
        <radialGradient id="lt1" cx="42%" cy="35%" r="65%">
          <stop offset="0%" stopColor="#FFF2F8"/><stop offset="55%" stopColor="#F8DAE6"/><stop offset="100%" stopColor="#D9BCF0"/>
        </radialGradient>
      </defs>
    ...
    <svg viewBox="0 0 52 52" {...common}>
      <defs>
        <linearGradient id="lt2" x1="0" y1="0" x2="1" y2="1">
          <stop offset="0%" stopColor="#E9D4F5"/><stop offset="100%" stopColor="#B297E8"/>
        </linearGradient>
      </defs>
    ...
    <svg viewBox="0 0 52 52" {...common}>
      <defs>
        <linearGradient id="lt3" x1="0" y1="1" x2="1" y2="0">
          <stop offset="0%" stopColor="#F2CFDD"/><stop offset="100%" stopColor="#B297E8"/>
        </linearGradient>
      </defs>
      <rect width="52" height="52" rx="14" fill="url(#lt3)"/>
      <path d="M6 38 Q18 20 28 32 T 48 22" stroke="#fff" strokeWidth="2.5" strokeLinecap="round" fill="none" opacity="0.75"/>
    </svg>
  ), g4: (
    <svg viewBox="0 0 52 52" {...common}>
      <defs>
        <radialGradient id="lt4" cx="55%" cy="50%" r="55%">
          <stop offset="0%" stopColor="#F3E5FA"/><stop offset="100%" stopColor="#B9A0E5"/>
        </radialGradient>
      </defs>
    ...
    <svg viewBox="0 0 52 52" {...common}>
      <defs>
        <linearGradient id="lt5" x1="0" y1="0" x2="1" y2="1">
          <stop offset="0%" stopColor="#FFE1ED"/><stop offset="100%" stopColor="#D4A8E8"/>
        </linearGradient>
      </defs>
