// Рекомендации — matches reference

function RecommendationsScreen({ onOpen, onClose }) {
  return (
    <div className="screen">
      <TopBar onClose={onClose} />
      <div className="screen-scroll">
        <h1 className="t-h1 c-primary" style={{ margin: '0 0 var(--sp-4)' }}>Рекомендации</h1>

        {/* Info banners — bare sparkle icon, no tinted container */}
        ...
        <div className="card tappable" style={{ padding: 'var(--sp-4)', marginTop: 'var(--sp-3)' }}
             onClick={() => onOpen('today-load')}>
          <div className="flex items-center" style={{ gap: 'var(--sp-3)' }}>
            <IconSparkle size={26} />
            <div className="t-body-md c-secondary flex-1">
              Сегодня лучше обойтись без высокой нагрузки.
            </div>
          </div>
        </div>

        {/* Meditation — orb left, content left-aligned, arrow chip right */}
        <div className="card tappable section-gap" style={{ padding: 'var(--sp-4)' }}
          ...
          <div className="flex items-center" style={{ gap: 'var(--sp-4)' }}>
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="t-body-sm c-tertiary" style={{ marginTop: 4 }}>Аудио-практика · 10 минут</div>
            </div>
            <ArrowChip onClick={(e) => { e.stopPropagation(); onOpen('meditation'); }} />
          </div>
        </div>

        {/* Material — book icon left */}
        <div className="card tappable" style={{ padding: 'var(--sp-4)', marginTop: 'var(--sp-3)' }}
          ...
          <div className="flex items-center" style={{ gap: 'var(--sp-4)' }}>
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="t-title-md c-primary" style={{ marginTop: 2 }}>
                Как питание влияет на<br/>организм и возраст
              </div>
              <div className="t-body-sm c-tertiary" style={{ marginTop: 6 }}>6 минут</div>
            </div>
            <ArrowChip onClick={(e) => { e.stopPropagation(); onOpen('material'); }} />
          </div>
        </div>

        {/* Dynamics — wave is full-width, baseline just above button */}
        <div className="card tappable" style={{ padding: 'var(--sp-4)', marginTop: 'var(--sp-3)', position: 'relative', overflow: 'hidden' }}
...
// Sparkle icon — purple flat
function IconSparkle({ size = 26 }) {
  return (
    <svg width={size} height={size} viewBox="0 0 26 26" fill="none">
      <path d="M13 3l2 5.3 5.3 2-5.3 2-2 5.3-2-5.3-5.3-2 5.3-2L13 3z" fill="#8E63F8"/>
      <path d="M5 17l1 2.5 2.5 1-2.5 1-1 2.5-1-2.5-2.5-1 2.5-1L5 17z" fill="#C09EFF"/>
    </svg>
  );
}

// Open book icon — purple
...
// Small circular arrow chip
function ArrowChip({ onClick }) {
  return (
    <button className="arrow-chip" onClick={onClick} aria-label="Открыть">
      <IconChevR size={14} sw={2.4} />
    </button>
  );
}

// Dynamics chart wave, right side of card
function DynamicsWave() {
  return (
    <svg style={{ position: 'absolute', left: 0, right: 0, top: 0, width: '100%', height: '100%', pointerEvents: 'none' }}
         viewBox="0 0 340 72" preserveAspectRatio="none">
      <defs>
      ...
      <path d="M0 48 Q60 32 120 42 T 220 18 T 340 36" fill="none" stroke="#F58BC8" strokeOpacity="0.75" strokeWidth="2"/>
    </svg>
  );
}

Object.assign(window, { RecommendationsScreen, ArrowChip });
